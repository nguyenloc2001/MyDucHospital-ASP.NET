...
vnTRUST.is_username = function(value) {
    return (value.match(/^[0-9]/) == null) && (value.search(/^[0-9_a-zA-Z]*$/) > -1);
}

vnTRUST.is_link = function(str) {
    return (/(http|https):\/\/(\w+:{0,1}\w*@)?(\S+)(:[0-9]+)?(\/|\/([\w#!:.?+=&%@!\-\/]))?/).test(vnTRUST.util_trim(str))
};
...
vnTRUST.is_ff = function() {
    return (/Firefox/).test(navigator.userAgent)
};

vnTRUST.is_ie = function() {
    return (/MSIE/).test(navigator.userAgent)
};

vnTRUST.is_ie6 = function() {
    return (/MSIE 6/).test(navigator.userAgent)
};

vnTRUST.is_ie7 = function() {
    return (/MSIE 7/).test(navigator.userAgent)
};

vnTRUST.is_ie8 = function() {
    return (/MSIE 8/).test(navigator.userAgent)
};

vnTRUST.is_chrome = function() {
    return (/Chrome/).test(navigator.userAgent)
};

vnTRUST.is_opera = function() {
    return (/Opera/).test(navigator.userAgent)
};

vnTRUST.is_safari = function() {
        ...
        success: function(xhr) {
            ...
            if (xhr && vnTRUST.is_exists(xhr.intReturn)) {
                switch (xhr.intReturn) {
                    case -1:
                        vnTRUST.show_popup_message(xhr.msg, "Thông báo lỗi!", -1);
                        break;
                    case 0:
                        vnTRUST.show_popup_message(xhr.msg, "Cảnh báo", 0);
                        break;
                    case 1:
                        vnTRUST.show_popup_message(xhr.msg, "Thông báo", 1);
                        break;
                }
            }
...
vnTRUST.hide_popup = function(id) {
    vnTRUST._hide_popup(id)
};
...
vnTRUST.confirm = function(message, callback, callback_data) {
    ...
    vnTRUST.show_next_popup(
        "popup_confirm",
        "Xác nhận",
        vnTRUST.join('<div style="font-weight: bold; margin: 0 0 10px;">' + message + '</div>')
...
vnTRUST.util_trim = function(str) {
    return (/string/).test(typeof str) ? str.replace(/^\s+|\s+$/g, "") : ""
};

vnTRUST.util_random = function(a, b) {
    return Math.floor(Math.random() * (b - a + 1)) + a
};

vnTRUST.get_ele = function(id) {
    return document.getElementById(id)
};

vnTRUST.get_uuid = function() {
...
vnTRUST.create3DText = function(id, deep, mainColor, overColor, left) {
    ...
    html = vnTRUST.join('<div class="txt3D" id="txt3D' + u + '" style="position:relative;z-index:0;width:' + w + 'px;height:' + h + 'px;">')
...
vnTRUST.deleteCache = function(cacheKey) {
    vnTRUST.ajax_popup("act=admin&code=delcache", 'POST', {
            cacheKey: cacheKey
        },
        function(j) {
            if (j.err == 0) {
                vnTRUST.cart.theme.systemAlert(
                    vnTRUST.join('<div style="font-size:14px;margin-top:5px">Xoá cache thành công</div>')(), 'Hệ thống'
                );
            }
        });
};
...
vnTRUST.error = {
    set: function(id, msg, width, cl) {
        ...
        var html = vnTRUST.join('<div class="my_msg" style="width: ' + width + 'px; color:red; margin: 5px auto 15px; padding:10px; background:rgb(255, 249, 215); border: 1px solid rgb(226, 200, 34); text-align: center; font-size: 15px;">')
            (msg)
            ('</div>')();
        if (cl) {
...
vnTRUST.timerObject = {
    ...
    countTime: function(id) {
        ...
        if (vnTRUST.timerObject.displayTime(id)) {
            vnTRUST.timerObject.obj[id].clock_id = setTimeout(function() {
                vnTRUST.timerObject.countTime(id)
            }, 1000);
        } else {
...
vnTRUST.popupSite = function(id, title, content, close, opt) {
    ...
    return vnTRUST.join('<div class="classic-popup"' + style + '>')
